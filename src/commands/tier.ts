...
module.exports = new Command(
  new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("닉네임").setDescription("검색할 닉네임을 입력하세요.").setRequired(true)
    )
    .addStringOption((option) =>
      option.setName("버튼").setDescription("검색할 버튼을 선택하세요.").setRequired(true).addChoices(
        {
          name: "4B",
          value: "4",
        },
  ...
  async (message: ChatInputCommandInteraction, bot: Bot) => {
    ...
    const result: UserTier = await fetch(`https://v-archive.net/api/archive/${nickname}/tier/${button}`).then((res) =>
      res.json()
    );

    if (!result.success) {
      await message.editReply({
        embeds: [new EmbedBuilder().setColor("#ff0000").setTitle(":warning: 오류").setDescription(result.message)],
      });
      return;
    }
